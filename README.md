...
## Usage
The first command line parameter is the input file, and the second is the output file. There
are no other command line paramters. Be careful not to mix up the input and output files, as this could
overwrite your program! Sample: **./Assembler my_instructions.s a.out**. If no output file is
specified, **a.out** will be used as the default file output name.
